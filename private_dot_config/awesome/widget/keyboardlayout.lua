--- ATTENTION: THE FOLLOWING IS STRAIGHT UP COPY PASTED FROM AWM
--- You needed a small implementation change
---------------------------------------------------------------------------
...
local function update_status(self)
    ...
    if #self._layout > 0 then
        ...
        if name then
            -- This is literally the only change you made lol
            if name == "us(dvorak)" then
                text = " " .. "dv" .. " "
            else
                text = " " .. name .. " "
            end
    ...
    self.widget:set_markup_silently("<span color='" .. self.text_color .. "'>" .. text .. "</span>")
end
...
function keyboardlayout.new(args)
    ...
    if args and args.text_color then
        self.text_color = args.text_color
    end

    if args and args.font then
