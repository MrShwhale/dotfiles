...
-- local lockscreen_background = os.getenv("HOME") .. "/Pictures/Backgrounds/sekai/mizuki/blurred/117_at_blurred.png"
local lockscreen_background = os.getenv("HOME") .. "/Pictures/Backgrounds/Landscapes/blurred/purple_water_blurred.jpg"

-- Parent folder the profile images
-- This should contain the files "thinking.png" and "wrong_"
-- TODO make these miku as well
local profile_image_folder = os.getenv("HOME") .. "/Pictures/lockscreen_icons/miku/"

-- Specify the folder which has the photos to rotate through
-- Should only contain the images to be used and optionally folders (which will be ignored)
-- local profile_image_used = os.getenv("HOME") .. "/Pictures/lockscreen_icons/mizuki/" .. "used/"
local profile_image_used = profile_image_folder .. "used/"
-- }}}

-- Profile image set
local profile_images = {}
-- Index of currently selected image
local profile_image_ind = 1
-- Total number of images in the set
local profile_image_len = 0

-- Lockscreen keygrabber
local ls_keygrabber

-- Fake password to be displayed as the real is typed
local fake_pass = "これは私のパスワードじゃあないです"

-- Default password text
-- Do not delete ellipsis, it makes everything work... sigh
local def_pass = "…  " .. "<span foreground=\"#808080\">パスワード</span>"
...
local function create_input(lockbox_color, s)
    ...
    local input_box = wibox.widget {
        ...
        bg = lockbox_color,
        fg = "#000000FF",
        shape = function(cr, width, height) gears.shape.rounded_rect(cr, width, height, 35) end,
        point = { x = s.geometry.x + 775, y = s.geometry.y + 570 },
        forced_width = 370,
        forced_height = 60,
        wibox.widget {
...
local function create_images(s, lockbox_color)
    ...
    for image in io.popen("ls -pa " .. profile_image_used .. " | grep -v /"):lines() do
        profile_image_len = profile_image_len + 1
        profile_images[profile_image_len] = image
    end
    ...
    s.lockscreen.profile_widget = profile_widget

    local profile_box = wibox.widget {
        widget = wibox.container.background,
        bg = lockbox_color,
        shape = function(cr, width, height) gears.shape.rounded_rect(cr, width, height, 20) end,
        point = { x = s.geometry.x + 775, y = s.geometry.y + 140 },
        forced_width = 370,
        forced_height = 370,
        profile_widget
    }

    profile_widget:connect_signal("button::release", function(self, lx, ly, button)
        -- CONS add secret middle mouse thing
        if button == 1 or button == 4 then
            profile_image_ind = (profile_image_ind == profile_image_len) and (1) or (profile_image_ind + 1)
            for s in screen do
                s.lockscreen.profile_widget.image = gears.surface.load_uncached(profile_image_used ..
                    profile_images[profile_image_ind])
            end
        elseif button == 3 or button == 5 then
            profile_image_ind = (profile_image_ind == 1) and (profile_image_len) or (profile_image_ind - 1)
            for s in screen do
                s.lockscreen.profile_widget.image = gears.surface.load_uncached(profile_image_used ..
                    profile_images[profile_image_ind])
            end
    ...
    return background_widget, profile_box
end
...
local function create_lockscreen(s)
    ...
    local background_widget, profile_box = create_images(s, lockbox_color)
...
try_login = function()
    -- Shows it is handling the password
    for s in screen do
        -- CONS making this a variable
        s.lockscreen.profile_widget.image = gears.surface.load_uncached(profile_image_folder .. "thinking.png")
    end
    ...
    gears.timer {
        ...
        callback = function()
            if pam.auth_current_user(table.concat(input)) then
                ...
                -- TODO add webcam with notification and option to delete photos from notif
                for s in screen do
                    s.lockscreen.profile_widget.image = gears.surface.load_uncached(profile_image_folder .. "wrong.png")
                end
            end
...
function lockscreen.show_lockscreen()
    ...
    -- Clear old image
    local new_image_index = math.random(1, #profile_images)
    local profile_image = gears.surface.load_uncached(profile_image_used .. profile_images[new_image_index])
    for s in screen do
        s.lockscreen.visible = true
        s.lockscreen.profile_widget.image = profile_image
    end
